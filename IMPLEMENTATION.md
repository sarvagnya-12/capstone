...
| 5 | GAN-centric framing vs. Stable Diffusion | Primary and only generative model in the MVP is a **GAN**. **Revised 2026-09-24: the architecture is FastGAN (ICLR 2021), not StyleGAN2** — see the revision note below Decision #6. No diffusion model is built. | Preserves the certified "GAN-centric" title per the standing instruction not to silently pivot the core narrative. The 2026-09-24 revision changes the architecture *within* GANs and so does not touch that framing. Stable Diffusion inclusion is not part of this plan at any phase; if desired later, it is a distinct decision requiring separate approval, not something this document schedules. |
| 6 | Training strategy vs. certified hardware (§21/§28) | ~~Transfer learning / fine-tuning from a pretrained StyleGAN2 checkpoint, on a rented cloud GPU.~~ **Revised 2026-09-24: train a FastGAN from scratch on 25,023 real product photos, on the local machine.** No pretrained weights are used. | Original rationale: reconciles the literature survey's warnings that StyleGAN2-class models are computationally heavy with the modest certified hardware spec. That reasoning was overtaken by evidence — see the revision note below. |

### Revision note (2026-09-24) — Decisions #5 and #6

Transfer learning was attempted three times and failed three times, each diagnosed rather than guessed at:

| # | Configuration | Outcome |
|---|---|---|
| 1 | No regularization, 512px, 3,014 images, lr 1e-3 | Discriminator domination (`d_loss` 0.0005). FID **262 → 466** |
...
Attempt 3 is the decisive one: stable, no NaN, and monotonically worse anyway. `finetune.py`'s own checkpoint selector reported `Best FID 324.99 at step 0` — the untouched pretrained checkpoint beat every fine-tuned one it produced.

**The obstacle was the domain gap, not hyperparameters.** The only pretrained checkpoints available are faces (`ffhq-res256`) and cats (`afhqcat`); carrying either to footwear is more than a few thousand steps of a batch-4 loop can do.

**What changed since Decision #6 was written:** 25,023 real 256×256 product photos now exist (Step 13's corpus did not). That is well above the 100–4,000 images modern lightweight GAN architectures target, so training from scratch — assumed infeasible on local hardware — became the *better* option, and it removes the domain gap entirely.

**Result:** FastGAN trained from scratch reached **FID 287.7 at step 14,000, down from 380.9**, and produces recognisably shoe-shaped output with laces, soles and realistic colour variety. Full record at Step 17.

**Scope of the revision:** "GAN-centric" is untouched — this is one GAN architecture replacing another. The StyleGAN2 path (`model.py`, `finetune.py`, `storage/models/afhqcat.pkl`) is deliberately left working and selectable via `settings.GAN_CHECKPOINT`, so the superseded approach stays reproducible rather than deleted from the record.
...
**Status: ✅ Completed for real (2026-09-24).** A trained generator now produces recognisable footwear, and the application generates from it by default.

**Superseded first pass (2026-08-17):** the fine-tuning script was written and smoke-tested only — no meaningful training run, per the then-standing instruction to defer training quality.

**Real outcome (2026-09-24): FastGAN trained from scratch, FID 380.9 → 287.7.**

Transfer learning from a pretrained StyleGAN2 checkpoint was attempted three times and beaten by its own starting point every time; the diagnosis and the decision to change architecture are recorded in the revision note under Decisions #5/#6. What finally worked:

- **Training corpus built from scratch.** The 46,269 ingested Zappos images are 136×102 thumbnails — fine-tuning a 256/512px model on them means upscaling 4× and teaching the generator to reproduce blur. Instead, `_ingestion_tools/fetch_amazon_training_images.py` downloads **25,023 photos at 256×256 from the ~1500px `hi_res` URLs already in the Amazon metadata** (a downscale, so detail is preserved). Non-square shots are **padded with white rather than centre-cropped**: product photos are overwhelmingly wide, cropping cut the toe and heel off the shoe, and an earlier aspect-ratio filter rejected 39 of 60 candidates for that reason. Padding is invisible here — sampling confirmed 25 of 25 images have all four corners at exactly (255,255,255).
- **Architecture**: FastGAN via `lightweight-gan==1.2.1`, 256×256, batch 8, lr 2e-4, differentiable augmentation (translation + cutout). No pretrained weights: G and D start from random initialisation.
- **Result**: 20,000 steps in ~2.5 hours on the local RTX 4050. FID fell 380.9 → **287.7 at step 14,000**, then rose to ~328 by 18,000 — training is *not* monotonic, so `settings.GAN_CHECKPOINT` points at the best checkpoint (`model_28.pt`, step 14,000), not the last. Output shows laces, soles, tongues and heel counters across black/red/tan/blue colourways; roughly 20% of samples are still malformed, which is expected at this scale and honestly reported rather than cropped out of sample grids.
- **Integration**: `app/ml/gan/fastgan.py` adapts the FastGAN generator to the surface `inference.py` already expects (`.z_dim`, plus a `(3, 256, 256)` `[0,1]` image), so Step 16's latent-perturbation variant generation, Step 18's FID scoring, persona simulation and orchestration all work unchanged. `load_generator()` dispatches on file extension (`.pt` → FastGAN, `.pkl` → StyleGAN2).

**Integration bugs found by testing, not by reading:**
- **Hand-constructing `LightweightGAN` + `load_state_dict` silently produced garbage.** Every key name matched, nothing raised, only non-persistent blur buffers were reported missing — and the model generated blue blobs on a tan field. The Trainer builds the network with parameters beyond those in `.config.json`, so the hand-built model was a different architecture wearing identical key names. Fixed by loading through the package's own `Trainer.load()`. Caught only by generating an image and looking at it.
- **FastGAN output is unbounded** (measured `[-2.40, 1.95]`), not `[0,1]`. StyleGAN2's `(img + 1) / 2` rescale applied on top would brighten every pixel, so `model.generate_image_from_latent()` dispatches by generator type. A regression test asserts an exact mid-range value survives unchanged; an earlier version of that test checked only min/max and **passed with the bug deliberately injected**, so it was rewritten until a mutation test confirmed it fails.
- **Four defects in `lightweight-gan` itself**, each worked around and documented at the call site: `use_aim=True` by default crashes because its own `ImportError` handler still dereferences the missing module; `num_train_steps` falls through `**kwargs` into a model that rejects it; `calculate_fid_every=0` raises `ZeroDivisionError` instead of disabling; and the FID helper imports `pytorch_fid` lazily, so a missing optional dependency **killed a 2.5-hour run at step 2,000**. Evaluation failures are now non-fatal and `--resume` exists.
- **`num_workers` is a 10× lever.** The package defaults it to `None`, making JPEG decode rather than the GPU the bottleneck: **4.55 s/step versus 0.45 s/step with 4 workers** — the difference between a 22-hour run and a 2.4-hour one.
...
**Status: ✅ Completed and fully verified (2026-09-23).** The verification gap disclosed below was closed by adding a second provider rather than by buying API access — see "Gap closed" at the end of this step.
...
- **Verification gap, disclosed rather than hidden** *(closed 2026-09-23 — kept here as the record of what was outstanding)*: the plan requires both a mocked-provider check *and* "a manual, real call against the configured provider" for full completion. I asked whether to add a real `ANTHROPIC_API_KEY`, and the user chose to skip real verification for now. **The mocked-provider path is fully verified** (prompt construction with real persona/scenario data; response parsing for clean JSON, markdown-fenced JSON, out-of-range clamping, and malformed input all confirmed correct; the "no API key configured" error path also confirmed to fail clearly rather than silently). **The real end-to-end API call has not been performed** — `get_llm_provider().complete()` against the live Anthropic API is unverified.

**Gap closed (2026-09-23) — second provider, not a purchased key.** A Claude Pro/Max subscription does not include API usage (verified against Anthropic's own docs), so rather than open separate per-token billing, the gap was closed by implementing the **`OllamaProvider`** the project had already recorded as a future option — a locally-hosted open-source model, which PRD §14 explicitly permits in place of an API. This resolves the "either an API or an open-source model" choice in favour of the latter without touching the `LLMProvider` Protocol, `persona_service.py`, or `prompt_templates.py`'s parser.
- **Concrete setup**: Ollama 0.34.3 serving `llama3.1:8b` (4.9GB, Q4) on `http://localhost:11434`; `LLM_PROVIDER` now defaults to `"ollama"` so the persona engine works with no key and no billing. `OLLAMA_BASE_URL`/`OLLAMA_MODEL` added to `Settings` and `.env.example`. `AnthropicProvider` is untouched and still selectable. No new pip dependency — `requests` was already pinned.
- **Stronger guarantee than the Anthropic path**: Ollama's `format` parameter takes a JSON schema and constrains decoding, so the model *cannot* emit anything but `{reaction_text, purchase_likelihood}`. The schema (`PERSONA_REACTION_SCHEMA`) lives in `prompt_templates.py` beside `parse_persona_reaction()` so the two can't drift, and is injected by `get_llm_provider()` rather than hardcoded in the provider, keeping the provider domain-agnostic. The existing code-fence-stripping parser stays as a harmless safety net; it is now belt-and-braces rather than load-bearing.
- **New tests**: `backend/tests/test_llm_provider.py` — 7 mocked unit tests (schema is actually sent, `stream: false`, trailing-slash handling, and that a connection failure or 404 becomes an actionable `LLMConfigurationError` naming the fix rather than a bare socket error) plus 1 live integration test that auto-skips when no Ollama is reachable, so CI stays green. Full suite: **62 passed**.
- **One existing test had to be repaired, not deleted**: `test_simulate_persona_reactions_without_llm_key_produces_marked_error_rows` created its "unconfigured provider" condition by clearing `ANTHROPIC_API_KEY`, which only worked because `"anthropic"` was the default. With `"ollama"` as the default that premise silently evaporated — on this machine the test made a real call and passed for the wrong reason. It now pins `LLM_PROVIDER="anthropic"` explicitly, so it tests the behaviour it claims to regardless of the default or what's installed locally.
- **Measured**: ~2.6–3.4s per call warm, ~49s on the first call (loading the model into VRAM on an RTX 4050). A 6-call simulation took 31.7s end to end.
...
**Status: ✅ Completed and fully verified (2026-09-23)** — including the persona-differentiation check that was previously impossible; see "Gap closed" at the end of this step.
...
**Gap closed (2026-09-23).** Re-run against the real `OllamaProvider` (Step 20's "Gap closed" note) with 3 contrasting seeded personas × 2 deliberately opposed variants (neon green / oversized logo vs. charcoal matte suede / minimal branding) at a premium price point. Results, read back directly from the `feedback` table:
- **6 rows, and 0 containing `[LLM_ERROR]`** — the first real persona reactions this system has ever produced. (Baseline before this run: 60 of 60 existing feedback rows were `[LLM_ERROR]`.)
- **The differentiation criterion genuinely holds, and is coherent rather than merely different**: Budget Shopper (APAC) rated the neon variant **0.05**, explicitly citing price and the oversized logo; Established/Luxury (US) rated the same variant 0.40 but the charcoal suede one **0.70**, citing its "understated elegance"; Athletic/Active sat between the two. Every persona preferred charcoal to neon. This is exactly the "a price-sensitive persona should mention price; a premium persona should not react the same way" check the Verification section below specifies.
- **6 distinct `purchase_likelihood` values** (0.05 / 0.20 / 0.40 / 0.50 / 0.60 / 0.70) and 6 distinct reaction texts — no clustering or template repetition.
- **`check_consistency()` (Step 22) now measures something real** instead of skipping for want of a provider: 3 of 6 rows sampled at `sample_rate=0.5`, `consistency_variance` populated with 0.050 / 0.550 / **1.000**. Reported as measured, not smoothed: that 1.000 is a genuinely large drift on re-run, which is the honest expected behaviour of a temperature-0.8 8B model and precisely the kind of evaluator-reliability signal Decision #7 added this check to surface. Lowering the temperature would reduce it at the cost of persona variety — a real tradeoff, not a bug to paper over.
- **Timing**: 31.7s for 6 calls (~5.3s each including a partial cold start). A full orchestrated run (5 personas × 3 variants × up to 3 iterations ≈ 45 calls) therefore lands in the low single-digit minutes, comfortably inside Step 28's background-task design.
...
**Status: ⚠️ Configuration complete and locally verified; the actual live deployment was not performed by me** — the user chose "I'll click through the provider's UI myself" as the execution model (no Render account credentials were shared, appropriately), so `docs/deployment.md` is a real, followable runbook rather than a record of a deployment I actually watched succeed. Disclosed honestly, not glossed over.

**Platform: Render**, chosen over raw AWS/Azure/GCP for this project's scale — one Blueprint (`render.yaml`) deploys all three pieces with minimal manual infrastructure work, real free tier, no separate load-balancer/networking setup needed.

**Two real Render-specific problems found by actually reading Render's current documentation (fetched live) before writing the config, not assumed from Step 41's local Docker setup:**
1. **Render's internal service-to-service hostname is not predictable before first deploy** (confirmed via Render's own private-networking docs: it's an auto-generated identifier like `elasticsearch-2j3e`, only visible in the dashboard *after* the service exists) — so Step 41's local `nginx.conf` proxy pattern (`proxy_pass http://backend:8000`, relying on docker-compose's own internal DNS) cannot be ported directly; hardcoding a guessed hostname would silently fail. Re-designed the frontend service as a Render **static site** (`runtime: static`) instead of a second Docker+nginx service, calling the backend's public URL directly with CORS enabled (`CORS_ORIGINS`, already fully built and tested) rather than needing same-origin proxying at all. The local `docker compose up` path is completely unaffected — it keeps using the original nginx-proxy `frontend/Dockerfile`, unchanged.
2. **Render's public service URL also isn't confirmed-predictable from the `render.yaml` name field** before first deploy (checked Render's docs specifically for this guarantee and found it explicitly *not* confirmed) — so `render.yaml` ships with clearly-marked placeholder values for the two settings that depend on knowing the other service's real URL (`CORS_ORIGINS` on the backend, `VITE_API_BASE_URL` on the frontend), and `docs/deployment.md`'s Step 2 is an explicit, unavoidable manual step: copy both real URLs from the dashboard after first deploy, set each service's corresponding value, and (since a static site bakes its API URL in at *build* time, not runtime) manually trigger a frontend redeploy. This is honest about a real limitation of blueprint-based deployment rather than presenting an untested guess as fact.

**A third issue, found and fixed in actual application code, not just deployment config**: Render's managed Postgres `fromDatabase` wiring hands back a bare `postgresql://` connection string, but this project only has `psycopg` (v3) installed, not `psycopg2` — and SQLAlchemy's bare `postgresql://` scheme defaults to looking for `psycopg2`, which would fail at engine-creation time. Every hand-written `.env` in this project already used the explicit `postgresql+psycopg://` form, masking this gap until a managed-provider connection string was actually considered. Fixed with a small, backward-compatible `field_validator` on `Settings.DATABASE_URL` (`backend/app/core/config.py`) that normalizes a bare `postgresql://`/`postgres://` prefix to `postgresql+psycopg://` — verified against 3 cases directly (already-correct URL passes through unchanged; both bare forms get normalized), and the full 54-test suite still passes unchanged, confirming no existing behavior broke.

**Also added**: the GAN checkpoint has no host filesystem to bind-mount in the cloud the way Step 41's local `docker-compose.yml` does — `backend/entrypoint.sh` now downloads it automatically from NVIDIA's public URL on first start if it's missing (idempotent; a near-instant no-op on every start after the first), paired with a Render persistent Disk mounted at `/app/storage` in `render.yaml` so it isn't re-downloaded on every restart (a paid-plan-only feature, documented clearly in `docs/deployment.md` along with what happens on the free tier without one).

**What was actually verified, locally, before considering this ready to hand off**: `render.yaml`'s exact field names and syntax (service `runtime: docker`/`runtime: static`, `dockerfilePath`/`dockerContext`, `envVars` with `fromDatabase`/`sync: false`/`generateValue`, `disk`, static-site `routes` with `action: rewrite`) were checked against Render's own current documentation (fetched live in this session, not recalled from training data) rather than guessed; the file parses as valid YAML with the expected service/database names; the `DATABASE_URL` normalization fix was unit-verified and the full local test suite re-run clean afterward; `entrypoint.sh`'s new download step reuses `requests` (already a dependency, avoiding a new system package like curl/wget). **What was not verified**: an actual live deploy on Render's infrastructure — whether the Blueprint applies cleanly, whether the free-tier disk restriction surfaces the way documented, whether the static site's rewrite rule behaves exactly as expected in practice. `docs/deployment.md` is written as a real runbook for the user to follow and report back on, not a summary of a deployment already confirmed working end-to-end.
