...
class IntelligenceExporterTests(unittest.TestCase):
    def test_creates_version_dirs(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            schema = root / "schema.json"
            schema.write_text(json.dumps({"columns": [{"name": "intelligence_record_id"}]}), encoding="utf-8")
            exporter = IntelligenceExporter(schema, root / "versions", root / "processed", root / "quarantine", root / "reports", root / "metadata")
