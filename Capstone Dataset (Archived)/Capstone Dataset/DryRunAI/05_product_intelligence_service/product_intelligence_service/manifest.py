...
class ProjectManifestUpdater:
    ...
    def update(self, intelligence_record_count: int) -> None:
        if self.manifest_path.exists():
            manifest = read_json(self.manifest_path)
        else:
            manifest = {"completed_services": [], "last_updated_timestamp": None}
        completed = set(manifest.get("completed_services", []))
        completed.add("05_product_intelligence_service")
        manifest["completed_services"] = sorted(completed)
        manifest["product_intelligence_record_count"] = int(manifest.get("product_intelligence_record_count", 0)) + int(
