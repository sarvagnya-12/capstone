...
from market_service.models import ProductMarketLink
...
class MarketProductLinker:
    def __init__(self, products_schema_path: Path) -> None:
        self.products_schema_path = products_schema_path

    def load(self, products_csv_path: Path) -> tuple[dict[str, ProductMarketLink], dict[str, ProductMarketLink], dict[str, ProductMarketLink]]:
        ...
        for row in rows:
            ...
            if link.source_product_id:
                by_source_product_id[self.key(link.source, link.source_product_id)] = link
            if link.source_url:
                by_source_url[link.source_url.strip().lower()] = link
        return by_product_id, by_source_product_id, by_source_url

    @staticmethod
    def key(source: str | None, source_product_id: str) -> str:
